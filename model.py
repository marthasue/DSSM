...
def build_query_model(features, mode):
    ...
    char_input = tf.reshape(features["host_char"], [-1, FLAGS.query_max_char_length])
...
def build_doc_model(features, mode):
    # 输入shape: [batch_size, sentence_size]
    char_input = tf.reshape(features["guest_char"], [-1, FLAGS.doc_max_char_length])
...
def model_fn(features, labels, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
        if FLAGS.export_query_model:
            ...
            predictions = {"host_vector": query_encoder}
        elif FLAGS.export_doc_model:
            doc_encoder = build_doc_model(features, mode)
            predictions = {"guest_vector": doc_encoder}
